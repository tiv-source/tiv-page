...
<struts:url var="sliderAddUrl" action="addForm" namespace="/others/slider" />
<struts:url var="remoteurl" action="table" namespace="/others/slider"/>
...
function formatPicture(cellvalue, options, rowObject) {
	return "<img src='/image/slider/" + cellvalue + "/thumbnail.png'/>";  
      ...
      <div class="main">
        <div class="sub_menu">
          <struts:a href="%{sliderAddUrl}">
            <struts:text name="slider.add"/>
          </struts:a>
        </div>
...
$(function () {
    $("#entityList").jqGrid({
        url: "/admin/others/slider/table.html",
        datatype: "json",
        mtype: "GET",
        colNames: [
            '<struts:text name="slider.image"/>',
            '<struts:text name="slider.name"/>',
            '<struts:text name="slider.clickable"/>',
            '<struts:text name="slider.orderNumber"/>',
        ...
        colModel: [
            { name: "image.uuid",                        width:  140, align: "center", formatter:formatPicture },
            { name: "name",                              width:  140, align: "right" },
            { name: "clickable",                         width:  140, align: "right", formatter:formatTrueFalse },
        ...
        cellLayout : 5,
        height:'auto',
        caption: '<struts:text name="slider"/>'
    }); 
}); 
</script>
